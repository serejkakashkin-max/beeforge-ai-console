...
The optimization page begins with a bounded, read-only GGUF v2/v3 little-endian metadata reader based on ggml's [format specification](https://github.com/ggml-org/ggml/blob/master/docs/gguf.md). It skips tokenizer arrays and tensor payloads, exposes only small architecture/configuration fields, and rejects malformed counts and truncation. Big-endian files and future structural versions are reported as unsupported; VRAM estimates are deliberately not inferred from a few metadata fields. The reader never writes to a model or profile.
