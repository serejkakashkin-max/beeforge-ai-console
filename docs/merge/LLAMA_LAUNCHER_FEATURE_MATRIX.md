...
| Auto optimizer | `OptimizationService`, samplers/pruners | Not present | Next preview: isolated baseline and bounded batch/ubatch/threads/threads-batch/flash trials, plus GPU-layer/CPU-MoE candidates when source metadata permits; PP/TG/balanced objectives, confirmation, minimum 5% improvement, optional backed-up copy of profile. Upstream TPE/pruners, KV/override quality checks and VRAM-aware search pending |
