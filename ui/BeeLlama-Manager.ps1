   ...
   <TabControl Grid.Column="1" Name="Tabs" Background="#181B21" Foreground="White" BorderBrush="#343B46">
    ...
    <TabItem Header="Доступ с ноутбука" Name="RemoteAccessTab">
     <ScrollViewer VerticalScrollBarVisibility="Auto"><StackPanel Margin="12">
      ...
      <Border Background="#302C20" BorderBrush="#8E7937" BorderThickness="1" CornerRadius="6" Padding="11"><TextBlock Text="Кнопка «Включить доступ» при необходимости сама запускает выбранную модель и ждёт READY перед передачей ноутбуку. Telegram на ноутбуке установщик RemoteClient не включает." TextWrapping="Wrap" Foreground="#FFE4A3"/></Border>
     </StackPanel></ScrollViewer>
    </TabItem>
...
function Refresh-SerenaMemoryView {
    try {
        ...
        $verified=@($script:serenaMemoryProjects|Where-Object{$_.Quality-eq'5/5'}).Count
        (UI 'SerenaMemorySummary').Text="Найдено проектов: $($script:serenaMemoryProjects.Count) • память готова: $ready • проверено полностью: $verified"
        (UI 'SerenaMemoryStatus').Text='Обычная задача сразу идёт профильному исполнителю. Память пополняется лениво; полный onboarding через Solution Architect выполняется только по явному запросу.'
